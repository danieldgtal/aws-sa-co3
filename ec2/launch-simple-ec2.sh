...
aws ec2 run-instances \
  ...
  --user-data file://ec2/install-kind-cluster.sh \
...
scp -i ~/.ssh/your-key.pem install-kind.sh ubuntu@$INSTANCE_IP:/tmp/ && ssh -i ~/.ssh/your-key.pem ubuntu@$INSTANCE_IP "chmod +x /tmp/install-kind.sh && sudo /tmp/install-kind.sh"  